...
from project.datasets.maf.util import one_hot_encode, logit, disp_imdata, logistic
...
class CIFAR10:
    ...
    class Data:
        ...
        def __init__(self, x, l, logit, flip, dequantize, rng):
            ...
            self.x = x.astype(np.float32)  # pixel values
    ...
    def __init__(self, logit=False, flip=False, dequantize=True):
        ...
        path = os.path.join(os.path.dirname(__file__), "data/cifar10/")
    ...
    def inv_logit_transform(self, z):
        return (logistic(z) - self.alpha) / (1 - 2 * self.alpha)
